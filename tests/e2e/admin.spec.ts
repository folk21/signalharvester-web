...
test('Sources renders configured data and creates a source with the expected request', async ({ page }) => {
...
test('Collection Runs shows durable detail and sends manual-run context', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (url.pathname === '/api/v1/admin/collection-runs' && request.method() === 'POST') {
      expect(request.postDataJSON()).toEqual({
        ...
        informationCategory: startedCollectionRunFixture.informationCategory,
      });
  ...
  await page.getByLabel('Monitoring profile ID').fill(startedCollectionRunFixture.monitoringProfileId);
  await page.getByLabel('Information category').fill(startedCollectionRunFixture.informationCategory);
  await page.getByRole('button', { name: 'Start collection run' }).click();

  await expect(page.getByText(startedCollectionRunFixture.monitoringProfileId, { exact: true }).first()).toBeVisible();
